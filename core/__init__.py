...
from core.config.config import MailConfig, DevelopmentConfig, ProductionConfig
...
def create_app():
    ...
    with app.app_context():
        ...
        from core.services.components.payment_processor import process_payments_with_context
        scheduler = APScheduler()
        scheduler.init_app(app)
        scheduler.add_job(id='process_payments', func=process_payments_with_context, trigger='interval', hours=6)
        scheduler.start()

        from .home.routes import landing_bp
        from .auth.routes import auth_bp
        from .services import accounts_bp
        from .services.components.accounts_context import accounts_context, inject_now
...
#        mail.init_app(app)
        ...
        from .database.models import MailSettings
        app.config.from_object(MailConfig)
        try:
            mail_settings = MailSettings.query.first()
